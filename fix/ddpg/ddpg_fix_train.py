...
log_find_nums = open("G:/decrease_model/fix/ddpg/ddpg_fix_findnums.csv", "w+")
...
def save_model_callback(_locals, _globals):
    ...
    if current_step % save_interval == 0:
        # 设置模型保存路径和文件名
        model_save_path = f"G:/decrease_model/saved_model/ddpg/ddpg_fix/ddpg_fix_{current_step:.1e}.zip"
        # 保存模型
...
model = DDPG("MlpPolicy", env=env, gamma=0.98, action_noise=action_noise, policy_kwargs=policy_kwargs, learning_rate=0.0001,
            learning_starts=500, tensorboard_log="G:/decrease_model/result/ddpg_fix", verbose=1, device=device)
