...
log_find_nums = open("G:/decrease_model/fix/ddpg/ddpg_lstm_att_fix_findnums.csv", "w+")
...
def save_model_callback(_locals, _globals):
    ...
    if current_step % save_interval == 0:
        # 设置模型保存路径和文件名
        model_save_path = f"G:/decrease_model/saved_model/ddpg/ddpg_lstm_att_fix/ddpg_lstm_att_fix_{current_step:.1e}.zip"
        # 保存模型
...
model.save("ddpg_lstm_att_fix.zip")
