...
log_find_nums = open("D:/decrease_model/random/td3/td3_random_supervised_findnums.csv", "w+")
log_ep_rew_mean = open("D:/decrease_model/random/td3/td3_random_supervised_ep_rew_mean.csv", "w+")
log_ep_len_mean = open("D:/decrease_model/random/td3/td3_random_supervised_ep_len_mean.csv", "w+")

Agent_actor_loss = open("D:/decrease_model/random/td3/Agent_actor_loss.csv", "w+")
Agent_critic_loss = open("D:/decrease_model/random/td3/Agent_critic_loss.csv", "w+")
...
def save_model_callback(_locals, _globals):
    ...
    if current_step % save_interval == 0:
        # 设置模型保存路径和文件名
        model_save_path = f"D:/decrease_model/saved_model/td3/td3_random/td3_random_{current_step:.1e}.zip"
        # 保存模型
...
agent_supervised = TD3("MlpPolicy", env=xietong_bu0_env(), gamma=0.98, action_noise=action_noise, policy_kwargs=policy_kwargs, learning_rate=0.0001,
            learning_starts=500, tensorboard_log="D:/decrease_model/result/td3_random_supervised", verbose=1, device=device)
...
agent_supervised.save("td3_random.zip")
