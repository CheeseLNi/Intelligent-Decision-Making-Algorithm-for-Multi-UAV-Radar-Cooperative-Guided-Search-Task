...
log_find_nums = open("./maddpg/maddpg_fix_4_nums.csv", "w+")
log_ep_rew_mean = open("./maddpg/maddpg_fix_4_rew.csv", "w+")
log_ep_len_mean = open("./maddpg/maddpg_fix_4_len.csv", "w+")
...
class single_bu0_env(gym.Env):
    ...
    def __init__(self):
        ...
        self.observation_space = spaces.Box(low=-100, high=100, shape=(80,), dtype=np.float16)

        self.target_num = 40  # 目标数目
        self.xietong_num = 1    # 协同数目
        self.search_range_x = [20, 20, 20, 20]  # 方位搜索范围
        self.search_range_y = [20, 20, 20, 20]  # 俯仰搜索范围

        self.x_m = np.array([-4.69362289, -11.6550148, 18.68441783, 45.54265673,
                             -9.90457537, -26.9257904, 18.90698969, 3.70223055,
                             -47.14336963, -49.73715558, -52.71816047, -7.96833861,
                             -14.18859317, -50.77131648, 21.50477526, 23.07467031,
                             29.77520887, -12.47022659, 17.30273577, 36.36743679,
                             39.82728303, 20.27002813, -1.03709845, 22.40328009,
                             42.08765033, -5.90545658, 29.30694534, 21.90328698,
                             -28.68781198, -23.59803249, 41.85910548, -59.99698927,
                             28.19776848, -49.53436496, -34.00058147, -48.84766942,
                             -57.96836999, -35.57381812, 2.8184726, -35.97094561])
        self.y_m = np.array([-22.73769328, -8.31075911, -9.14956126, -53.14624522,
                             33.39774289, -47.58838689, -20.07841801, -3.76898514,
                             -44.08179432, -29.09682898, -37.58987503, 5.12236989,
                             -42.35753686, 48.70417983, -17.30402171, 21.76263196,
                             56.77117242, -6.1989533, -56.45058095, -56.38685435,
                             -55.72731966, 44.59883701, 30.55890517, 19.57831029,
                             -11.23901774, -24.98075867, 57.31281241, -29.86823001,
                             -42.43729381, -48.44682043, -55.02778322, 57.69975997,
                             30.45572544, -3.44376835, 56.90878082, 46.26748284,
                             9.8824957, -55.45889285, -3.06328398, -51.68997007])
...
class xietong_env(gym.Env):
    ...
    def __init__(self):
        ...
        self.action_space = spaces.Box(low=-1.0, high=+1.0, shape=(8,), dtype=np.float16)
        self.observation_space = spaces.Box(low=-100, high=100, shape=(80,), dtype=np.float16)
...
class att_model(nn.Module):
    def __init__(self):
        super().__init__()
        self.my_embd1 = nn.Linear(in_features=80, out_features=n_embd * n_head)
...
def collect_xietong_rollouts(multi_agent, xietong_agent, multi_callback, xietong_callback):
    ...
    while should_collect_more_steps(xietong_agent.train_freq, num_collected_steps, num_collected_episodes):     # 这里用协同智能体进行回合判断
        ...
        #                 multi_rewards += 20.0
        multi_rewards += 2 * (Agent_num - 1)  # 这里相当于减了Agent_num个1，所以加回来
...
multi_agent = [agent1, agent2, agent3, agent4]
agent_num = np.size(multi_agent)
# for i in range(agent_num):
...
while xietong_agent.num_timesteps < total_timesteps:   # 三个智能体设置的训练步长是一样的
    ...
    # if step % 1000 == 0:
    #     multi_agent[0].save(f"D:/decrease_model/saved_model/maddpg/maddpg_lstm_att/maddpg_lstm_att_fix_actor1_{step:.1e}.zip")
    #     multi_agent[1].save(f"D:/decrease_model/saved_model/maddpg/maddpg_lstm_att/maddpg_lstm_att_fix_actor2_{step:.1e}.zip")
    #     multi_agent[2].save(f"D:/decrease_model/saved_model/maddpg/maddpg_lstm_att/maddpg_lstm_att_fix_actor3_{step:.1e}.zip")
    #     xietong_agent.save(f"D:/decrease_model/saved_model/maddpg/maddpg_lstm_att/maddpg_lstm_att_fix_critic_{step:.1e}.zip")
...
multi_agent[0].save("maddpg_fix_actor1")
multi_agent[1].save("maddpg_fix_actor2")
multi_agent[2].save("maddpg_fix_actor3")
multi_agent[3].save("maddpg_fix_actor4")
xietong_agent.save("maddpg_fix_critic")
