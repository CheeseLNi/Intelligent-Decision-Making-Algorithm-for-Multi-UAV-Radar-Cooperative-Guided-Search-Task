...
action_dim = 2
log_find_nums = open("./maddpg/ma_snf_6_30_nums.csv", "w+")
log_ep_rew_mean = open("./maddpg/ma_snf_6_30_rew.csv", "w+")
log_ep_len_mean = open("./maddpg/ma_snf_6_30_len.csv", "w+")
...
input_dataset = [input_data, input_data, input_data, input_data, input_data, input_data]
output_dataset = [output_data, output_data, output_data, output_data, output_data, output_data]
...
class single_bu0_env(gym.Env):
    ...
    def __init__(self):
        ...
        self.search_range_x = [20, 20, 20, 20, 20, 20]  # 方位搜索范围
        self.search_range_y = [20, 20, 20, 20, 20, 20]  # 俯仰搜索范围
        ...
        self._max_episode_steps = 60  # 单个最大搜索是30次
        self.t = 0  # 全局执行步长
...
class xietong_env(gym.Env):
    ...
    def __init__(self):
        ...
        self.action_space = spaces.Box(low=-1.0, high=+1.0, shape=(12,), dtype=np.float16)
        self.observation_space = spaces.Box(low=-100, high=100, shape=(60,), dtype=np.float16)
...
agent5 = DDPG("MlpPolicy", env=single_bu0_env(), gamma=0.98, action_noise=action_noise, policy_kwargs=policy_kwargs,
              learning_rate=0.0001, learning_starts=500, verbose=1, device=device)
agent6 = DDPG("MlpPolicy", env=single_bu0_env(), gamma=0.98, action_noise=action_noise, policy_kwargs=policy_kwargs,
              learning_rate=0.0001, learning_starts=500, verbose=1, device=device)
multi_agent = [agent1, agent2, agent3, agent4, agent5, agent6]
agent_num = np.size(multi_agent)
# 协同智能体动作空间为6，状态空间为30
