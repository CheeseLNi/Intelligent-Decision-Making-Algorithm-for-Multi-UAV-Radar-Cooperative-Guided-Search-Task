...
import random 
log_find_nums = open("D:/decrease_model/random/facmac/facmac_att_lstm_random_findnums.csv", "w+")
log_ep_rew_mean = open("D:/decrease_model/random/facmac/facmac_att_lstm_random_ep_rew_mean.csv", "w+")
log_ep_len_mean = open("D:/decrease_model/random/facmac/facmac_att_lstm_random_ep_len_mean.csv", "w+")

Agent1_actor_loss = open("D:/decrease_model/random/facmac/Agent1_actor_loss1.csv", "w+")
Agent2_actor_loss = open("D:/decrease_model/random/facmac/Agent2_actor_loss1.csv", "w+")
Agent3_actor_loss = open("D:/decrease_model/random/facmac/Agent3_actor_loss1.csv", "w+")
Agent_actor_loss = [Agent1_actor_loss, Agent2_actor_loss, Agent3_actor_loss]
Agent_critic_loss = open("D:/decrease_model/random/facmac/Agent_critic_loss1.csv", "w+")
...
agent1 = DDPG("MlpPolicy", env=single_bu0_env(), gamma=0.98, action_noise=action_noise, policy_kwargs=policy_kwargs, learning_rate=0.0001,
            learning_starts=500, tensorboard_log="D:\\result\\2023_12_18\\facmac_rd_AS_xietong_result1", verbose=1, device=device)
...
xietong_agent = DDPG("MlpPolicy", env=xietong_env(), gamma=0.98, action_noise=action_noise, policy_kwargs=policy_kwargs, learning_rate=0.0001,
            learning_starts=500, tensorboard_log="D:\\result\\2023_12_18\\facmac_rd_AS_xietong_result1", verbose=1, device=device)  # 单个智能体只用critic网络
...
while xietong_agent.num_timesteps < total_timesteps:   # 三个智能体设置的训练步长是一样的
    ...
    if step % 1000 == 0:
        multi_agent[0].save(f"D:/decrease_model/saved_model/facmac/facmac_att_lstm_random/facmac_att_lstm_random_actor1_{step:.1e}.zip")
        multi_agent[1].save(f"D:/decrease_model/saved_model/facmac/facmac_att_lstm_random/facmac_att_lstm_random_actor2_{step:.1e}.zip")
        multi_agent[2].save(f"D:/decrease_model/saved_model/facmac/facmac_att_lstm_random/facmac_att_lstm_random__actor3_{step:.1e}.zip")
        xietong_agent.save(f"D:/decrease_model/saved_model/facmac/facmac_att_lstm_random/facmac_att_lstm_random_critic_{step:.1e}.zip")
...
multi_agent[0].save("facmac_att_lstm_random_actor1")
multi_agent[1].save("facmac_att_lstm_random_actor2")
multi_agent[2].save("facmac_att_lstm_random_actor3")
xietong_agent.save("facmac_att_lstm_random_critic")
