...
import random 
log_find_nums = open("D:/decrease_model/random/facmac/facmac_att_random_supervised_findnums.csv", "w+")
log_ep_rew_mean = open("D:/decrease_model/random/facmac/facmac_att_random_supervised_ep_rew_mean.csv", "w+")
log_ep_len_mean = open("D:/decrease_model/random/facmac/facmac_att_random_supervised_ep_len_mean.csv", "w+")

Agent1_actor_loss = open("D:/decrease_model/random/facmac/Agent1_actor_loss.csv", "w+")
Agent2_actor_loss = open("D:/decrease_model/random/facmac/Agent2_actor_loss.csv", "w+")
Agent3_actor_loss = open("D:/decrease_model/random/facmac/Agent3_actor_loss.csv", "w+")
Agent_actor_loss = [Agent1_actor_loss, Agent2_actor_loss, Agent3_actor_loss]
Agent_critic_loss = open("D:/decrease_model/random/facmac/Agent_critic_loss.csv", "w+")

dataset_num = int(1e5)
Agent1_input_data = np.loadtxt('D:/decrease_model/random/facmac/log_Agent1_input.txt', delimiter=',', usecols=range(60), max_rows=dataset_num)
Agent2_input_data = np.loadtxt('D:/decrease_model/random/facmac/log_Agent2_input.txt', delimiter=',', usecols=range(60), max_rows=dataset_num)
Agent3_input_data = np.loadtxt('D:/decrease_model/random/facmac/log_Agent3_input.txt', delimiter=',', usecols=range(60), max_rows=dataset_num)
Agent1_output_data = np.loadtxt('D:/decrease_model/random/facmac/log_Agent1_output.csv', delimiter=',', usecols=range(2), max_rows=dataset_num)
Agent2_output_data = np.loadtxt('D:/decrease_model/random/facmac/log_Agent2_output.csv', delimiter=',', usecols=range(2), max_rows=dataset_num)
Agent3_output_data = np.loadtxt('D:/decrease_model/random/facmac/log_Agent3_output.csv', delimiter=',', usecols=range(2), max_rows=dataset_num)
...
def train_agent(multi_agent, xietong_agent, gradient_steps):
    ...
    for _ in range(gradient_steps):
        ...
        # FACMAC: 更新各动作智能体动作网络 - 每个智能体只基于自己的Q值优化
        if xietong_agent._n_updates % xietong_agent.policy_delay == 0:
            for i in range(Agent_num):
                # 为每个智能体创建新的动作张量，避免修改原始张量
                actions_i = replay_data.actions.clone()
                # 重新选择对应智能体动作
                action_rechoose = multi_agent[i].actor(replay_data.observations)
                # 替换当前智能体的动作到新的动作张量中
                actions_i[:, 2*i:2*(i+1)] = action_rechoose
                # 只获取当前智能体的Q值
                agent_q_value = xietong_agent.critic.q1_forward(replay_data.observations, actions_i)
                # 计算当前智能体的actor loss
                actor_loss = -agent_q_value.mean()
                # Optimize the actor
...
def train_xietong_agent_critic(multi_agent, xietong_agent, gradient_steps, step):   # 只更新评价网络
    ...
    for _ in range(gradient_steps):
        ...
        with torch.no_grad():
            ...
            # FACMAC: 为每个智能体计算单独的target_q_values
            next_q_values_list = []
            for i in range(Agent_num):
                # 获取针对当前智能体的Q值
                next_q_values_i = xietong_agent.critic_target.q1_forward(
                    replay_data.next_observations, multi_next_actions
                )
                next_q_values_list.append(next_q_values_i)
...
agent1 = DDPG("MlpPolicy", env=single_bu0_env(), gamma=0.98, action_noise=action_noise, policy_kwargs=policy_kwargs, learning_rate=0.0001,
            learning_starts=500, tensorboard_log="D:\\result\\2023_12_18\\facmac_rd_AS_xietong_result", verbose=1, device=device)
...
xietong_agent = DDPG("MlpPolicy", env=xietong_env(), gamma=0.98, action_noise=action_noise, policy_kwargs=policy_kwargs, learning_rate=0.0001,
            learning_starts=500, tensorboard_log="D:\\result\\2023_12_18\\facmac_rd_AS_xietong_result", verbose=1, device=device)  # 单个智能体只用critic网络
...
# multi_agent[0].load("facmac_att_fix_actor1_1e6.zip")
# multi_agent[1].load("facmac_att_fix_actor2_1e6.zip")
# multi_agent[2].load("facmac_att_fix_actor3_1e6.zip")
# xietong_agent.load("facmac_att_fix_critic_1e6.zip")
...
while xietong_agent.num_timesteps < total_timesteps:   # 三个智能体设置的训练步长是一样的
    ...
    if step % 1000 == 0:
        multi_agent[0].save(f"D:/decrease_model/saved_model/facmac/facmac_att_random_supervised/facmac_att_random_supervised_actor1_{step:.1e}.zip")
        multi_agent[1].save(f"D:/decrease_model/saved_model/facmac/facmac_att_random_supervised/facmac_att_random_supervised_actor2_{step:.1e}.zip")
        multi_agent[2].save(f"D:/decrease_model/saved_model/facmac/facmac_att_random_supervised/facmac_att_random_supervised_actor3_{step:.1e}.zip")
        xietong_agent.save(f"D:/decrease_model/saved_model/facmac/facmac_att_random_supervised/facmac_att_random_supervised_critic_{step:.1e}.zip")
...
multi_agent[0].save("facmac_att_random_supervised_actor1")
multi_agent[1].save("facmac_att_random_supervised_actor2")
multi_agent[2].save("facmac_att_random_supervised_actor3")
xietong_agent.save("facmac_att_random_supervised_critic")

# agent1.load("facmac_fix_actor1.zip")
# agent2.load("facmac_fix_actor2.zip")
# agent3.load("facmac_fix_actor3.zip")
# xietong_agent.load("facmac_fix_critic.zip")
# multi_agent = [agent1, agent2, agent3]
#
