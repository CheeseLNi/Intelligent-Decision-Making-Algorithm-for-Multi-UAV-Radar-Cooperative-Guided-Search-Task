...
log_find_nums = open("G:/decrease_model/random/td3_origin/td3_origin_random_findnums.csv", "w+")
...
def save_model_callback(_locals, _globals):
    ...
    if current_step % save_interval == 0:
        # 设置模型保存路径和文件名
        model_save_path = f"G:/decrease_model/saved_model/td3/td3_origin/td3_origin_random_{current_step:.1e}.zip"
        # 保存模型
...
model = TD3("MlpPolicy", env=env, gamma=0.98, action_noise=action_noise, policy_kwargs=policy_kwargs, learning_rate=0.0001,
            learning_starts=500, tensorboard_log="G:/decrease_model/result/td3_origin_random", verbose=1, device=device)
...
model.save("td3_origin_random.zip")
