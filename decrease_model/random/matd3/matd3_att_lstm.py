...
import random
# 添加监督学习所需日志
log_find_nums = open("D:/decrease_model/random/matd3/matd3_att_lstm_randomfindnums.csv", "w+")
log_ep_rew_mean = open("D:/decrease_model/random/matd3/matd3_att_lstm_random_ep_rew_mean.csv", "w+")
log_ep_len_mean = open("D:/decrease_model/random/matd3/matd3_att_lstm_random_ep_len_mean.csv", "w+")
Agent1_actor_loss = open("D:/decrease_model/random/matd3/Agent1_actor_lstm_loss.csv", "w+")
Agent2_actor_loss = open("D:/decrease_model/random/matd3/Agent2_actor_lstm_loss.csv", "w+") 
Agent3_actor_loss = open("D:/decrease_model/random/matd3/Agent3_actor_lstm_loss.csv", "w+")
Agent_actor_loss = [Agent1_actor_loss, Agent2_actor_loss, Agent3_actor_loss]
Agent_critic_loss = open("D:/decrease_model/random/matd3/Agent_critic_lstm_loss.csv", "w+")

# 加载监督学习数据集（需提前生成）
dataset_num = int(1e5)
Agent1_input_data = np.loadtxt('D:/decrease_model/random/matd3/log_Agent1_input.txt', delimiter=',', usecols=range(60), max_rows=dataset_num)
Agent2_input_data = np.loadtxt('D:/decrease_model/random/matd3/log_Agent2_input.txt', delimiter=',', usecols=range(60), max_rows=dataset_num)
Agent3_input_data = np.loadtxt('D:/decrease_model/random/matd3/log_Agent3_input.txt', delimiter=',', usecols=range(60), max_rows=dataset_num)
Agent1_output_data = np.loadtxt('D:/decrease_model/random/matd3/log_Agent1_output.csv', delimiter=',', usecols=range(2), max_rows=dataset_num)
Agent2_output_data = np.loadtxt('D:/decrease_model/random/matd3/log_Agent2_output.csv', delimiter=',', usecols=range(2), max_rows=dataset_num)
Agent3_output_data = np.loadtxt('D:/decrease_model/random/matd3/log_Agent3_output.csv', delimiter=',', usecols=range(2), max_rows=dataset_num)
...
class att_model(nn.Module):
    def __init__(self):
        ...
        self.relu = nn.ReLU()
        self.tanh = nn.Tanh()
        self.lstm_norm = nn.LayerNorm(n_embd)
    ...
    def _init_weights(self, module):
        ...
        for name, param in self.lstm.named_parameters():
            if 'weight_ih' in name:
                ...
                # 增强遗忘门偏置
                nn.init.constant_(param[n_embd:2*n_embd], 1.5)
    ...
    def forward(self, x):
        ...
        x = x_lstm + identity # 残差连接
        # 添加层标准化
        x = self.lstm_norm(x)
        x = self.blocks(x_lstm)
...
def train_agent_supervised(multi_agent, step):
    ...
    for i in range(len(multi_agent)):
...
agent1 = DDPG("MlpPolicy", env=single_bu0_env(), gamma=0.98, action_noise=action_noise, policy_kwargs=policy_kwargs, learning_rate=0.0001,
            learning_starts=500, tensorboard_log="D:\\decrease_model\\result\\2025_2_27\\lstm_AS_rd_xietong_result", verbose=1, device=device)
...
xietong_agent = MATD3Agent("MlpPolicy", env=xietong_env(), gamma=0.98, 
                          ...
                          tensorboard_log="D:\\decrease_model\\result\\2025_2_27\\lstm_AS_rd_xietong_result", device=device)
# 在智能体初始化参数中增加
...
critic_only_steps = 25000   # 仅Critic训练阶段步数
supervised_steps = 5000   # 监督学习阶段步数
# 初始化协同智能体的回调函数
for i in range(agent_num):
...
xietong_total_timesteps, xietong_callback = xietong_agent._setup_learn(
    total_timesteps, 
    callback, 
    reset_num_timesteps, 
    tb_log_name, 
...
while xietong_agent.num_timesteps < total_timesteps:
    step += 1
    # 阶段1：监督预训练 (0~5k步)
    if step <= supervised_steps:
         train_agent_supervised(multi_agent, step)
    # 阶段2：Critic专项训练 (5k~25k步)
    elif step < critic_only_steps and step > supervised_steps :
        # 正常收集经验
        rollout = collect_xietong_rollouts(multi_agent, xietong_agent, multi_callback, xietong_callback)
        # 仅更新Critic网络
        if xietong_agent.replay_buffer.size() > batch_size:
            critic_losses = []
            for _ in range(gradient_steps):
                # 采样批次
                replay_data = xietong_agent.replay_buffer.sample(batch_size)
                # 计算目标Q值（与MATD3相同）
                with torch.no_grad():
                    next_actions = []
                    for agent in multi_agent:
                        next_action = multi_agent[i].actor_target(replay_data.next_observations)
                        next_actions.append(next_action)
                    next_actions = torch.cat(next_actions, dim=1)
                    
                    target_Q1, target_Q2 = xietong_agent.critic_target(replay_data.next_observations, next_actions)  # ✅ 正确属性名
                    target_Q = torch.min(target_Q1, target_Q2)
                    target_Q = replay_data.rewards + (1 - replay_data.dones) * 0.98 * target_Q
                
                # 更新双Critic
                current_Q1, current_Q2 = xietong_agent.critic(replay_data.observations, replay_data.actions)
                critic_loss = F.mse_loss(current_Q1, target_Q) + F.mse_loss(current_Q2, target_Q)
                
                xietong_agent.critic.optimizers[0].zero_grad()
                xietong_agent.critic.optimizers[1].zero_grad()
                critic_loss.backward()
                xietong_agent.critic.optimizers[0].step()
                xietong_agent.critic.optimizers[1].step()
                critic_losses.append(critic_loss.item())
            # 记录损失
            avg_critic_loss = np.mean(critic_losses)
            Agent_critic_loss.write(f'{avg_critic_loss}\n')
            Agent_critic_loss.flush()
            if step % 100 == 0:
                print(f"Critic Only Loss: {avg_critic_loss:.4f}")
    # 阶段3：完整MATD3训练 (>25k步)
    else:
        # 收集经验
        rollout = collect_xietong_rollouts(multi_agent, xietong_agent, multi_callback, xietong_callback)
        ...
        if xietong_agent.replay_buffer.size() > batch_size:
            ...
            if step % 2 == 0:  # MATD3默认策略延迟
                ...
                for agent in multi_agent:
                    ...
                    # 计算策略梯度
                    actions = agent.actor(replay_data.observations)
                    new_actions = replay_data.actions.clone()
                    new_actions[:, :2] = actions  # 假设第一个Agent动作在前两位
                    
                    Q1, _ = xietong_agent.critic(replay_data.observations, new_actions)
                    actor_loss = -Q1.mean()
                ...
                for i, loss in enumerate(actor_losses):
                    Agent_actor_loss[i].write(f'{loss}\n')
                    Agent_actor_loss[i].flush()
                
                if step % 100 == 0:
                    print(f"Actor Losses: {np.mean(actor_losses):.4f}")
    
    # 模型保存与评估
    if step % 1000 == 0:
        multi_agent[0].save(f"D:/decrease_model/saved_model/matd3/matd3_random_lstm_supervised/matd3_att_random_lstm_supervised_actor1_{step:.1e}.zip")
        multi_agent[1].save(f"D:/decrease_model/saved_model/matd3/matd3_random_lstm_supervised/matd3_att_random_lstm_supervised_actor2_{step:.1e}.zip")
        multi_agent[2].save(f"D:/decrease_model/saved_model/matd3/matd3_random_lstm_supervised/matd3_att_random_lstm_supervised_actor3_{step:.1e}.zip")
        xietong_agent.save(f"D:/decrease_model/saved_model/matd3/matd3_random_lstm_supervised/matd3_att_random_lstm_supervised_critic_{step:.1e}.zip")
...
multi_agent[0].save("matd3_att_ran dom_lstm_supervised_actor1")
multi_agent[1].save("matd3_att_random_lstm_supervised_actor2")
multi_agent[2].save("matd3_att_random_lstm_supervised_actor3")
xietong_agent.save("matd3_att_random_lstm_supervised_critic")