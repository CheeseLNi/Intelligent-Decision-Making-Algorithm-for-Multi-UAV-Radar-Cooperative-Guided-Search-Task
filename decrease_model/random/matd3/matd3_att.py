...
import random
# 添加监督学习所需日志
log_find_nums = open("D:/decrease_model/random/matd3/matd3_att_randomfindnums.csv", "w+")
log_ep_rew_mean = open("D:/decrease_model/random/matd3/matd3_att_random_ep_rew_mean.csv", "w+")
log_ep_len_mean = open("D:/decrease_model/random/matd3/matd3_att_random_ep_len_mean.csv", "w+")
Agent1_actor_loss = open("D:/decrease_model/random/matd3/Agent1_actor_loss.csv", "w+")
Agent2_actor_loss = open("D:/decrease_model/random/matd3/Agent2_actor_loss.csv", "w+") 
Agent3_actor_loss = open("D:/decrease_model/random/matd3/Agent3_actor_loss.csv", "w+")
Agent_actor_loss = [Agent1_actor_loss, Agent2_actor_loss, Agent3_actor_loss]
Agent_critic_loss = open("D:/decrease_model/random/matd3/Agent_critic_loss.csv", "w+")
...
class MATD3Critic(nn.Module):
    def __init__(self, obs_dim, act_dim):
        ...
        # 统一优化器接口
        self.optimizers = [
            torch.optim.Adam(self.q1_net.parameters(), lr=1e-4),
            torch.optim.Adam(self.q2_net.parameters(), lr=1e-4)
        ]
...
agent1 = DDPG("MlpPolicy", env=single_bu0_env(), gamma=0.98, action_noise=action_noise, policy_kwargs=policy_kwargs, learning_rate=0.0001,
            learning_starts=500, tensorboard_log="D:\\decrease_model\\result\\2025_2_27\\AS_rd_xietong_result", verbose=1, device=device)
...
xietong_agent = MATD3Agent("MlpPolicy", env=xietong_env(), gamma=0.98, 
                          ...
                          tensorboard_log="D:\\decrease_model\\result\\2025_2_27\\AS_rd_xietong_result", device=device)
# 在智能体初始化参数中增加
...
while xietong_agent.num_timesteps < total_timesteps:
    ...
    # 阶段2：Critic专项训练 (5k~25k步)
        ...
        if xietong_agent.replay_buffer.size() > batch_size:
            ...
            for _ in range(gradient_steps):
                ...
                critic_loss.backward()
                torch.nn.utils.clip_grad_norm_(xietong_agent.critic.parameters(), 1.0)
    ...
    # 模型保存与评估
    if step % 1000 == 0:
        multi_agent[0].save(f"D:/decrease_model/saved_model/matd3/matd3_random_supervised/matd3_att_random_supervised_actor1_{step:.1e}.zip")
        multi_agent[1].save(f"D:/decrease_model/saved_model/matd3/matd3_random_supervised/matd3_att_random_supervised_actor2_{step:.1e}.zip")
        multi_agent[2].save(f"D:/decrease_model/saved_model/matd3/matd3_random_supervised/matd3_att_random_supervised_actor3_{step:.1e}.zip")
        xietong_agent.save(f"D:/decrease_model/saved_model/matd3/matd3_random_supervised/matd3_att_random_supervised_critic_{step:.1e}.zip")
...
multi_agent[0].save("matd3_att_random_supervised_actor1")
multi_agent[1].save("matd3_att_random_supervised_actor2")
multi_agent[2].save("matd3_att_random_supervised_actor3")
xietong_agent.save("matd3_att_random_supervised_critic")