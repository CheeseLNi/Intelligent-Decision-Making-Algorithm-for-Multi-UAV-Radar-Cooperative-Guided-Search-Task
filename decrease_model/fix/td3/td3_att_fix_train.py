...
log_find_nums = open("G:/decrease_model/fix/td3/td3_att_fix_findnums.csv", "w+")
...
class att_model(nn.Module):
    def __init__(self):
        ...
        self.lstm = nn.LSTM(n_embd, 64, n_layer, batch_first=True)
        self.relu = nn.ReLU()
        self.tanh = nn.Tanh()
...
def save_model_callback(_locals, _globals):
    ...
    save_interval = 1000  # 每10000步保存一次模型
    if current_step % save_interval == 0:
        # 设置模型保存路径和文件名
        model_save_path = f"G:/decrease_model/saved_model/td3/td3_att_fix/td3_att_fix_{current_step:.2e}.zip"
        # 保存模型
...
model = TD3("MlpPolicy", env=env, gamma=0.98, action_noise=action_noise, policy_kwargs=policy_kwargs, learning_rate=0.0001,
            learning_starts=500, tensorboard_log="G:/decrease_model/result/td3_att_fix", verbose=1, device=device)
